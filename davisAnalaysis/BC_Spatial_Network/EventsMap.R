...
# Step 2: Generate maps for each year with the common extent
for (year in years) {
  ...
  for (event_type in event_types) {
    ...
    # Filter top 10 largest clusters
    top_10_indices <- order(community_sizes, decreasing = TRUE)[1:10]
    top_10_nodes <- which(membership(communities) %in% top_10_indices)
    
    # Extract spatial data for top 10 clusters
    node_data <- data.frame(
      id = V(g)$name[top_10_nodes],
      longitude = V(g)$longitude[top_10_nodes],
      latitude = V(g)$latitude[top_10_nodes],
      community = factor(membership(communities)[top_10_nodes]),
  ...
  # Generate the map for the current year with the common bounding box
  map <- tm_basemap("OpenStreetMap") +
    ...
    tm_layout(
      title = paste(year, "Top 10 Communities in Each Event Type"),
      title.size = 2,
      legend.outside = TRUE
    )
  
  # Save the map as an HTML or PNG file for reference
  tmap_save(map, filename = paste0("Map_", year, ".html"))
