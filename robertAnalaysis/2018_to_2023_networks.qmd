...
The data we used can be accessed through: (https://catalogue.data.gov.bc.ca/dataset/historical-drivebc-events)[] my analysis on this page used the years 2018 to 2023 and I renamed the files to the format `drivebceventshistXXXX` where XXXX represents the year of the csv.
...
Created an adjacency matrix using distance between roads based on their maintenance/incident patterns
Converted to an igraph network where:
